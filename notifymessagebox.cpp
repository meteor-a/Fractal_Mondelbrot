...
NotifyMessageBox::NotifyMessageBox(const QString& text, int milliseconds, QWidget* parent)
    : QWidget(parent)
    , m_label(text)
    , m_opacity(1.0)
    , m_milliseconds(milliseconds)
...
void NotifyMessageBox::showMessage(const QString& message, int milliseconds, QWidget* parent)
{
    showMessage(message, FONT, milliseconds, parent);
}
